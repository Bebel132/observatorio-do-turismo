...
$log_filepath = '/tmp/debug-webhook-observatorio-do-turismo.iplanfor.fortaleza.ce.gov.br.log';
...
if ($payload = file_get_contents('php://input')) {
    ...
    $branch = 'master';
    $path = '/var/www/observatorio-do-turismo.fortaleza.ce.gov.br';

    if (isset($_GET["env"]) && $_GET["env"] === "testing") {
        $refBranch = 'refs/heads/develop';
        $branch = 'develop';
        $path = '/var/www/observatorio-do-turismo.iplanfor.fortaleza.ce.gov.br';
    }
    ...
    $op = array();
    exec("cd {$path} && git fetch origin && git reset --hard origin/{$branch} && git merge origin/{$branch}", $op);
